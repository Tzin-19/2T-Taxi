...
fun PassengerHomeScreen(onLogout: () -> Unit) {
    Scaffold(
        topBar = {
            CenterAlignedTopAppBar(title = { Text("Giao diện Khách hàng") })
        }
    ) { padding ->
        Column(
            modifier = Modifier
            ...
            Text(text = "Chào mừng Khách hàng!", style = MaterialTheme.typography.headlineMedium)
            Spacer(modifier = Modifier.height(16.dp))
            Text(text = "Bạn muốn đi đâu hôm nay?")
